...
// Check Google Drive MCP status from backend
async function checkGoogleDriveStatus() {
  ...
  try {
    const response = await fetch('http://localhost:3001/api/gdrive/status');
    ...
    if (data.enabled) {
      ...
      label.textContent = `Google Drive connected (${data.toolCount} tools)`;
    } else if (data.credentialsFound && !data.authenticated) {
      badge.classList.add('warning');
      label.textContent = 'Google Drive: needs authentication (run npm run auth)';
    } else if (!data.credentialsFound) {
      badge.classList.add('disabled');
      label.textContent = 'Google Drive: not configured';
    } else {
      badge.classList.add('disabled');
      label.textContent = 'Google Drive: disabled';
    }
  } catch (err) {
    badge.classList.remove('loading');
    badge.classList.add('disabled');
    label.textContent = 'Google Drive: backend not running';
  }
}
...
function formatToolPreview(toolInput) {
  ...
  const previewKeys = ['pattern', 'command', 'file_path', 'path', 'query', 'content', 'description', 'name', 'documentId', 'spreadsheetId', 'presentationId', 'range', 'folderId'];
